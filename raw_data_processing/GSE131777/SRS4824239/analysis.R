...
whole <- get(load("/mnt/tank/scratch/mfiruleva/scn/10_fq/GSE131777/SRS4824239/counts.RData"))
...
# file_out <- paste0(gsub('/SRR.*', '', stringr::str_extract("/mnt/tank/scratch/mfiruleva/scn/10_fq/GSE131777/SRS4824239/counts.RData", 'SRS.*')), '.RData')
...
target_path <- gsub('counts.RData', '', "/mnt/tank/scratch/mfiruleva/scn/10_fq/GSE131777/SRS4824239/counts.RData")
...
tech <- sample$technology

if (tech == '10x') {
  tech <- unique(stringr::str_extract(readr::read_file(paste0(target_path, 'kallisto.sh')), "10xv[1-3]"))
}

adata$uns["token"] = unique(sample$secondary_sample_accession)
adata$uns["species"] = unique(sample$scientific_name)
adata$uns["public"] = TRUE
adata$uns["curated"] = FALSE
adata$uns["gse"] = unique(sample$GSE)
...
adata$uns["sample"]
adata$uns["geo"] = paste0("https://www.ncbi.nlm.nih.gov/sra/", unique(sample$GSE))
...
adata$uns["technology"] = paste(tech,collapse=",")
adata$uns["cells"] = ncol(whole)
adata$uns['status'] = filter(brief,  annot=='!Series_status')$data
