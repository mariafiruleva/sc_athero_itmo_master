...
get_df <- function(path) {
  ...
  gse_id <- stringr::str_extract(path, 'GSE[0-9]*')
  df <- fread(sprintf('annotations/%s.tsv', gse_id))
  data$sample <- gsub('.*/', '', gsub('/counts.RData', '', path))
  data$dataset <- gse_id
  data$organism <- 'Mus musculus'
  for (col in colnames(df)[!grepl('GSE|SRS', colnames(df))]) {
    data[[col]] <- df[[col]][match(data$sample, df$SRS)]
  }
  data
}
...
whole <- get_whole_obj(c("/mnt/tank/scratch/mfiruleva/scn/data/GSE116240/SRS3458842/counts.RData",
