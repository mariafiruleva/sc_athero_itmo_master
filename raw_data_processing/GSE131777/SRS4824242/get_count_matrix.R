...
createSeurat <- function(counts) {
  ...
  cells.before <- ncol(data)
  e.out <- emptyDrops(data, lower = 100)
  ...
  ggplot(data = e.out, aes(x = Total, y = -LogProb))+
    ...
    ggtitle(paste(gsub("^.*?GSE","GSE", getwd())))+
  ...
  stats <- cbind(cells.before, cells.after, sample=gsub("^.*?GSE","GSE", getwd()))
  write.table(stats, file="/scratch/mfiruleva/winter/logs/stats.csv", append = T, col.names = F, sep=',', quote = F, row.names = F)
  obj
}
